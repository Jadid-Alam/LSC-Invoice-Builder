...
public class PdfBuilder {
    ...
    public PdfBuilder(Storage storage1) {
        ...
        try {
            ...
            for (int row = 0; row < storage.getNoOfChildren(); row++) {
                ...
                if (storage.getIsHoursThere()) {
                    ...
                    PdfPCell pphCell = new PdfPCell(new Phrase(storage.getStudentPPW(row) + "", normalFont));
                    pphCell.setPadding(tablePadding);
                    studentTable.addCell(pphCell);
                }

                PdfPCell fpwCell = new PdfPCell(new Phrase("£" + storage.getStudentFPW(row),normalFont));
            ...
            PdfPCell totalPaidValueCell = new PdfPCell(new Phrase("£"+totalP,normalFont));
            ...
            else if (storage.getRegistry().equals("EY2649415"))
            {
